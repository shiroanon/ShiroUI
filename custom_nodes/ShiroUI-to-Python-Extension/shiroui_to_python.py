...
import sys
import re
from typing import Dict, List, Any, Callable, Tuple, TextIO, Set
from argparse import ArgumentParser

import black


from shiroui_to_python_utils import (
    import_custom_nodes,
    find_path,
...
class ArgumentFinder:
    """Find all arguments starting with -- in the workflow JSON data."""

    @staticmethod
    def find_special_arguments(data: Dict) -> Set[str]:
        """
        Find all strings starting with -- in the workflow JSON.
        
        Args:
            data (Dict): The workflow JSON data.
            
        Returns:
            Set[str]: A set of unique argument names found.
        """
        special_args = set()
            ...
            if isinstance(d, dict):
                for k, v in d.items():
                    if isinstance(v, str) and v.startswith("--"):
                        # Extract the argument name without -- prefix
                        arg_name = v[2:]
                        special_args.add(arg_name)
                    else:
                        search_dict(v)
...
class CodeGenerator:
    ...
    Attributes:
        ...
        special_args (Set[str]): Set of special arguments found in the workflow.
    """

    def __init__(self, node_class_mappings: Dict, base_node_class_mappings: Dict, special_args: Set[str] = None):
        ...
        self.special_args = special_args or set()
    ...
    def format_arg(self, key: str, value: any) -> str:
        ...
        if key == "noise_seed" or key == "seed":
            ...
            # Handle special argument values that start with --
            arg_name = value[2:]  # Remove -- prefix
            return f"{key}=args.{arg_name}"
    ...
    def assemble_python_code(
        ...
        arg_parser_code = self.generate_argument_parser_code()
        
        # Assemble the main function code, including custom nodes if applicable
        main_function_code = (
            "def main():\n"
            + arg_parser_code
            + f"\n\t{custom_nodes_code}with torch.inference_mode():\n\t\t"
        ...
        try:
            ...
            # If Black formatting fails, return unformatted code
            pass

        return final_code
    
    def generate_argument_parser_code(self) -> str:
        """Generate code for parsing command line arguments with appropriate types.
        
        Returns:
            str: Code for argument parser.
        """
        if not self.special_args:
            return ""
        
        parser_code = "\t# Parse command line arguments\n"
        parser_code += "\tparser = argparse.ArgumentParser(description='Run ShiroUI workflow with custom arguments')\n"
        
        # Add each special argument to the parser
        for arg in sorted(self.special_args):
            # Determine argument type based on name patterns
            if any(dim in arg for dim in ["width", "height", "steps", "count", "seed", "num", "size"]):
                arg_type = "int"
                default_value = "512" if any(dim in arg for dim in ["width", "height"]) else "0"
            elif any(float_param in arg for float_param in ["strength", "scale", "weight", "ratio", "alpha", "beta", "sigma", "threshold"]):
                arg_type = "float"
                default_value = "0.0"
            else:
                arg_type = "str"
            ...
            parser_code += f"\tparser.add_argument('--{arg}', type={arg_type}, default={default_value}, help='{arg.replace('_', ' ')} parameter')\n"
        
        parser_code += "\targs = parser.parse_args()\n"
...
class ShiroUItoPython:
    ...
    def __init__(
        ...
        """Initialize the ShiroUItoPython class with the given parameters. Exactly one of workflow or input_file must be specified.
        Args:
    ...
    def execute(self):
        ...
        # Step 2.5: Find all special arguments in the JSON data
        special_args = ArgumentFinder.find_special_arguments(data)
        ...
        code_generator = CodeGenerator(
            self.node_class_mappings, self.base_node_class_mappings, special_args
        )
...
def run(
    ...
    """Generate Python code from a ShiroUI workflow_api.json file.
...
def main() -> None:
    """Main function to generate Python code from a ShiroUI workflow_api.json file."""
    parser = ArgumentParser(
        description="Generate Python code from a ShiroUI workflow_api.json file."
    )
    parser.add_argument(
        "-f",
        "--input_file",
