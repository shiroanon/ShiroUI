...
from shiro.samplers import KSampler
...
DEFAULT_SHIRO_GRAPH = ShiroGraph(graph=default_graph, sampler_nodes=['10','14'])
DEFAULT_SHIRO_GRAPH_ID = os.path.splitext(os.path.basename(default_graph_file))[0]

#
# Loop through these variables
#
shiro_graph_list = [DEFAULT_SHIRO_GRAPH]
shiro_graph_ids = [DEFAULT_SHIRO_GRAPH_ID]
prompt_list = [
    'a painting of a cat',
]
...
class TestInference:
    ...
    @fixture(scope="class", params=shiro_graph_list, ids=shiro_graph_ids, autouse=True)
    def _client_graph(self, request, args_pytest, _server) -> (ShiroClient, ShiroGraph):
        shiro_graph = request.param
        
        # Start client
        shiro_client = self.start_client(args_pytest["listen"], args_pytest["port"])

        # Warm up pipeline
        shiro_client.get_images(graph=shiro_graph.graph, save=False)

        yield shiro_client, shiro_graph
        del shiro_client
        del shiro_graph
        torch.cuda.empty_cache()
    ...
    @fixture
    def shiro_graph(self, _client_graph):
    ...
    def test_shiro(
        ...
        shiro_graph.set_filename_prefix(test_info)
        # Settings for shiro graph
        shiro_graph.set_sampler_name(sampler)
        shiro_graph.set_scheduler(scheduler)
        shiro_graph.set_prompt(prompt)

        # Generate
        images = client.get_images(shiro_graph.graph)
