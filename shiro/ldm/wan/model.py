...
from shiro.ldm.modules.attention import optimized_attention
from shiro.ldm.flux.layers import EmbedND
from shiro.ldm.flux.math import apply_rope
from shiro.ldm.modules.diffusionmodules.mmdit import RMSNorm
import shiro.ldm.common_dit
...
class WanAttentionBlock(nn.Module):
    ...
    def forward(
        ...
        e = (shiro.model_management.cast_to(self.modulation, dtype=x.dtype, device=x.device) + e).chunk(6, dim=1)
...
class Head(nn.Module):
    ...
    def forward(self, x, e):
        ...
        """
        # assert e.dtype == torch.float32
        e = (shiro.model_management.cast_to(self.modulation, dtype=x.dtype, device=x.device) + e.unsqueeze(1)).chunk(2, dim=1)
...
class WanModel(torch.nn.Module):
    ...
    def forward(self, x, timestep, context, clip_fea=None, **kwargs):
        bs, c, t, h, w = x.shape
        x = shiro.ldm.common_dit.pad_to_patch_size(x, self.patch_size)
