...
def prepare_mask(noise_mask, shape, device):
    return shiro.utils.reshape_mask(noise_mask, shape).to(device)
...
def get_hooks_from_cond(cond, full_hooks: shiro.hooks.HookGroup):
    ...
    extra_hooks = shiro.hooks.HookGroup.combine_all_hooks(hooks_list)
...
def get_additional_models_from_model_options(model_options: dict[str]=None):
    ...
    if model_options is not None and "registered_hooks" in model_options:
        registered: shiro.hooks.HookGroup = model_options["registered_hooks"]
        for hook in registered.get_type(shiro.hooks.EnumHookType.AdditionalModels):
            hook: shiro.hooks.AdditionalModelsHook
...
def prepare_sampling(model: ModelPatcher, noise_shape, conds, model_options=None):
    ...
    shiro.model_management.load_models_gpu([model] + models, memory_required=memory_required, minimum_memory_required=minimum_memory_required)
...
def prepare_model_patcher(model: 'ModelPatcher', conds, model_options: dict):
    ...
    hooks = shiro.hooks.HookGroup()
    for k in conds:
        get_hooks_from_cond(conds[k], hooks)
    # add wrappers and callbacks from ModelPatcher to transformer_options
    model_options["transformer_options"]["wrappers"] = shiro.patcher_extension.copy_nested_dicts(model.wrappers)
    model_options["transformer_options"]["callbacks"] = shiro.patcher_extension.copy_nested_dicts(model.callbacks)
    # begin registering hooks
    registered = shiro.hooks.HookGroup()
    target_dict = shiro.hooks.create_target_dict(shiro.hooks.EnumWeightTarget.Model)
    # handle all TransformerOptionsHooks
    for hook in hooks.get_type(shiro.hooks.EnumHookType.TransformerOptions):
        hook: shiro.hooks.TransformerOptionsHook
        hook.add_hook_patches(model, model_options, target_dict, registered)
    # handle all AddModelsHooks
    for hook in hooks.get_type(shiro.hooks.EnumHookType.AdditionalModels):
        hook: shiro.hooks.AdditionalModelsHook
        hook.add_hook_patches(model, model_options, target_dict, registered)
    ...
    for wc_name in ["wrappers", "callbacks"]:
        shiro.patcher_extension.merge_nested_dicts(to_load_options.setdefault(wc_name, {}), model_options["transformer_options"][wc_name],
                                                    copy_dict1=False)
