...
class TripleCLIPLoader:
    ...
    def load_clip(self, clip_name1, clip_name2, clip_name3):
        ...
        clip = shiro.sd.load_clip(ckpt_paths=[clip_path1, clip_path2, clip_path3], embedding_directory=folder_paths.get_folder_paths("embeddings"))
        return (clip,)
...
class SkipLayerGuidanceSD3:
    ...
    def skip_guidance(self, model, layers, scale, start_percent, end_percent):
        ...
        def post_cfg_function(args):
            ...
            for layer in layers:
                model_options = shiro.model_patcher.set_model_options_patch_replace(model_options, skip, "dit", "double_block", layer)
            model_sampling.percent_to_sigma(start_percent)

            sigma_ = sigma[0].item()
            if scale > 0 and sigma_ >= sigma_end and sigma_ <= sigma_start:
                (slg,) = shiro.samplers.calc_cond_batch(model, [cond], x, sigma, model_options)
