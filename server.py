...
def get_shiroui_version():
    shiroui_version = "unknown"
    repo_path = os.path.dirname(os.path.realpath(__file__))
    try:
        import pygit2
        repo = pygit2.Repository(repo_path)
        shiroui_version = repo.describe(describe_strategy=pygit2.GIT_DESCRIBE_TAGS)
    except Exception:
        try:
            import subprocess
            shiroui_version = subprocess.check_output(["git", "describe", "--tags"], cwd=repo_path).decode('utf-8')
        except Exception as e:
            logging.warning(f"Failed to get ShiroUI version: {e}")
    return shiroui_version.strip()
...
def create_origin_only_middleware():
    ...
    async def origin_only_middleware(request: web.Request, handler):
        #this code is used to prevent the case where a random website can queue shiro workflows by making a POST to 127.0.0.1 which browsers don't prevent for some dumb reason.
    ...
    def __init__(self, loop):
        ...
        async def view_metadata(request):
            ...
            if safetensors_path is None:
                return web.Response(status=404)
            out = shiro.utils.safetensors_header(safetensors_path, max_size=1024*1024)
            if out is None:
                return web.Response(status=404)
            dt = json.loads(out)
        ...
        async def system_stats(request):
            ...
            device_name = shiro.model_management.get_torch_device_name(device)
            cpu_device = shiro.model_management.torch.device("cpu")
            ram_total = shiro.model_management.get_total_memory(cpu_device)
            ram_free = shiro.model_management.get_free_memory(cpu_device)
            vram_total, torch_vram_total = shiro.model_management.get_total_memory(device, torch_total_too=True)
            vram_free, torch_vram_free = shiro.model_management.get_free_memory(device, torch_free_too=True)
            ...
            system_stats = {
                "system": {
                    ...
                    "ram_total": ram_total,
                    "ram_free": ram_free,
                    "shiroui_version": get_shiroui_version(),
                    "python_version": sys.version,
                    "pytorch_version": shiro.model_management.torch_version,
